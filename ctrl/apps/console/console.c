...
static void start_action(console_cfg_t *console);
static int console_proc(console_cfg_t *console);

status_t console_start(console_cfg_t *console)
{
  if(console == NULL) return status_error;
...
{
  int argc, cmd_ret;
  console_cfg_t *console = (console_cfg_t *)argument;
  start_action(console);
  console->console_tx((uint8_t *)"\r\nkyChu Shell\r\n", 15);
  for(;;) {
    // start new command line
    console->console_tx((uint8_t *)console->prompt, strlen(console->prompt));
    if(console_proc(console) > 0) {
      argc = shell_split_argv((char *)console_cache->ln_cache, argv, CONSOLE_MAX_CMDLINE_ARGS);
      if(shell_cmd_run(argc, argv, &cmd_ret) == status_not_found) {

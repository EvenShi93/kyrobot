...
static void config_bl_btn_evt_cb(int id, btn_evt_type_t evt);
...
struct btn_cb_t config_bl_btn_evt = {
  config_bl_btn_evt_cb,
  NULL
};
...
//        ky_info("BL", "set backlight to %d%%", val);
      break;
      }
    break;
    }
...
void gui_config_backlight_start(void)
{
  ...
  xSize = LCD_GetXSize();
  ySize = LCD_GetYSize();
  hSlider = SLIDER_CreateEx(0, (ySize - BL_SLIDER_HEIGHT) >> 1, xSize, BL_SLIDER_HEIGHT, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, 0, GUI_ID_SLIDER0);
  ...
  btn_evt_register_callback(&config_bl_btn_evt);
  do {
    GUI_Delay(100);
    if(auto_dec_inc == 1) {
      SLIDER_Inc(hSlider);
  ...
  btn_evt_unregister_callback(&config_bl_btn_evt);
  auto_dec_inc = 0;
  should_exit = 0;

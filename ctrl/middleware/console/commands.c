...
#define CONSOLE_COMMAND_LOG_ENABLE               (0)

#if CONSOLE_COMMAND_LOG_ENABLE
#include "log.h"

static const char *TAG = "CMD";
#endif /* CONSOLE_COMMAND_LOG_ENABLE */
...
static const console_cmd_t *find_command_by_name(const char *name);
...
status_t console_cmd_register(const console_cmd_t *cmd)
{
  ...
  if ((console_cmd_t *)find_command_by_name(cmd->command) == NULL) {
...
{
  const console_cmd_t *cmd = find_command_by_name(argv[0]);
  if (cmd == NULL) {

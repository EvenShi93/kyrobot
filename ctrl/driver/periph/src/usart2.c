...
static status_t usart2_init(uint32_t baudrate);
static status_t usart2_deinit(void);
static status_t usart2_tx_bytes(uint8_t *p, uint32_t l);
static status_t usart2_tx_bytes_it(uint8_t *p, uint32_t l);
static status_t usart2_tx_bytes_dma(uint8_t *p, uint32_t l);

static uint32_t usart2_rx_bytes(uint8_t *p, uint32_t l);
static uint32_t usart2_cache_usage(void);

static const char *TAG = "ttyS2";

const uart_dev_t usart2_dev = {
//  "ttyS2",
  usart2_init,
  usart2_deinit,
  usart2_rx_bytes,
